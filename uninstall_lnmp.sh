#!/bin/bash
# =============================================================================
# LNMP 一键卸载脚本 (Debian / RHEL / CentOS)
# 功能：卸载 Nginx、MySQL、PHP 及相关配置
# =============================================================================
...
LNMP_CMD="/usr/local/bin/lnmp"
LOGROTATE_CONF="/etc/logrotate.d/lnmp"
...
uninstall_nginx() {
    ...
    read -p "是否删除所有网站目录 ${WEB_PATH}? (y/N): " del_web
    if [[ "$del_web" =~ ^[Yy]$ ]]; then
        if [[ -d "$WEB_PATH" ]]; then
            # 移除所有 .user.ini 的不可变属性
            find "$WEB_PATH" -name ".user.ini" -exec chattr -i {} \; 2>/dev/null || true
            rm -rf "$WEB_PATH"
    ...
    if [[ "$del_user" =~ ^[Yy]$ ]]; then
        if pgrep -u www-data >/dev/null 2>&1; then
            log_warn "www-data 用户下有运行中的进程，跳过删除"
        else
...
uninstall_mysql() {
    ...
    if [[ -f "/etc/init.d/mysql" ]]; then
        ...
        for rcdir in /etc/rc*.d; do
            if [[ -d "$rcdir" ]]; then
                find "$rcdir" -maxdepth 1 -type l -name "*mysql*" -exec rm -f {} \; 2>/dev/null || true
            fi
        done
    ...
    if [[ "$del_user" =~ ^[Yy]$ ]]; then
        if pgrep -u mysql >/dev/null 2>&1; then
            log_warn "mysql 用户下有运行中的进程，跳过删除"
        else
...
uninstall_php() {
    ...
    # 清理符号链接
    rm -f /usr/bin/php /usr/bin/phar /usr/bin/phpize /usr/bin/php-config /usr/sbin/php-fpm 2>/dev/null || true
    rm -f /usr/local/bin/php /usr/local/bin/phar /usr/local/bin/phpize /usr/local/bin/php-config 2>/dev/null || true
    
    # 清理环境变量配置
    if [[ -f "/etc/profile.d/lnmp.sh" ]]; then
        rm -f /etc/profile.d/lnmp.sh
        log_success "已删除环境变量配置: /etc/profile.d/lnmp.sh"
    fi
...
uninstall_logrotate() {
    ...
    if [[ -f "$LOGROTATE_CONF" ]]; then
        rm -f "$LOGROTATE_CONF"
        log_success "已删除日志轮转配置: $LOGROTATE_CONF"
    fi
    log_success "日志轮转配置清理完成"
}
...
show_menu() {
    while true; do
        ...
        echo "3. 卸载 PHP"
        echo "4. 卸载日志轮转配置"
        echo "5. 一键卸载全部 (Nginx + MySQL + PHP + lnmp命令 + 日志轮转)"
        echo "6. 仅卸载 lnmp 命令"
        echo "0. 退出"
        echo "========================================"
        read -p "请选择要卸载的组件 [0-6]: " choice
        case $choice in
            1)
                uninstall_nginx
                ;;
